...
SYNWORKDIR_ESF=${SYNWORKDIR}/ESF
GoodDecon=${SRCDIR}/Decon.txt
BinFile=${SRCDIR}/Bins.txt
...
case "${DeconMethod}" in
	...
	Waterlevel )
		...
		SYNWORKDIR_FRS=${SYNWORKDIR}/WaterFRS
	...
	Ammon )
		...
		SYNWORKDIR_FRS=${SYNWORKDIR}/AmmonFRS
	...
	WaterWL )
		...
		SYNWORKDIR_FRS=${SYNWORKDIR}/WaterWLFRS
	...
	Subtract )
		...
		SYNWORKDIR_FRS=${SYNWORKDIR}/SubtractFRS
	...
	Raw )
		...
		SYNWORKDIR_FRS=${SYNWORKDIR}/RawFRS
	...
	WaterHalfS )
		...
		SYNWORKDIR_FRS=${SYNWORKDIR}/WaterHalfSFRS
	...
	WaterS )
		...
		SYNWORKDIR_FRS=${SYNWORKDIR}/WaterSFRS
...
keys1="<EQ> <F1> <F2>"
keys2="<EQ> <S_E1> <S_E2> <ScS_E1> <ScS_E2>"
keys3="<EQ> <WBegin> <WLen> <WBegin_ScS> <WLen_ScS>"
${BASHCODEDIR}/Findfield.sh ${CODEDIR}/EQ_Parameters "${keys1}" > ${WORKDIR}/tmpfile1_$$
${BASHCODEDIR}/Findfield.sh ${CODEDIR}/EQ_Parameters "${keys2}" > ${WORKDIR}/tmpfile2_$$
${BASHCODEDIR}/Findfield.sh ${CODEDIR}/EQ_Parameters "${keys3}" > ${WORKDIR}/tmpfile3_$$
for EQ in ${EQnames}
do
    grep ${EQ} ${WORKDIR}/tmpfile1_$$ >> ${WORKDIR}/EQ_Freq_$$
    grep ${EQ} ${WORKDIR}/tmpfile2_$$ >> ${WORKDIR}/EQ_ESW_$$
    grep ${EQ} ${WORKDIR}/tmpfile3_$$ >> ${WORKDIR}/EQ_Peak_$$
done
...
# Flags.
[ -e ${GoodDecon} ] && flag_goodfile=1 || flag_goodfile=0
[ -e ${BinFile} ] && flag_bin=1 || flag_bin=0
[ ${UseGoodDecon} -eq 0 ] && flag_goodfile=0
[ ${UseBins} -eq 0 ] && flag_bin=0
