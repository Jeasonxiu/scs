...
do
    ...
    echo "    ==> Plotting dt Source Distribution of ${EQ}..."
...
EOF
	...
	mysql -N -u shule ${DB} > tmpfile_az_takeoff_dTS_Fast << EOF
select az,asin(Srayp*${Vs}/(6371.0-evde))*180.0/PI(),-1.0*D_T_S_All/6 from Master_a10 where eq=${EQ} and wantit=1 and D_T_S_All<=0;
EOF
	mysql -N -u shule ${DB} > tmpfile_az_takeoff_dTS_Slow << EOF
select az,asin(Srayp*${Vs}/(6371.0-evde))*180.0/PI(),D_T_S_All/6 from Master_a10 where eq=${EQ} and wantit=1 and D_T_S_All>0;
EOF
	mysql -N -u shule ${DB} > tmpfile_az_takeoff_dTScS_Fast << EOF
select az,asin(ScSrayp*${Vs}/(6371.0-evde))*180.0/PI(),-1.0*D_T_ScS_All/6 from Master_a10 where eq=${EQ} and wantit=1 and D_T_ScS_All<=0;
EOF
	mysql -N -u shule ${DB} > tmpfile_az_takeoff_dTScS_Slow << EOF
select az,asin(ScSrayp*${Vs}/(6371.0-evde))*180.0/PI(),D_T_ScS_All/6 from Master_a10 where eq=${EQ} and wantit=1 and D_T_ScS_All>0;
EOF
