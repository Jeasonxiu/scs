...
ncdump HMSL-S06_percent.nc > all.ncdump

# velocity data
d1=`grep -n "dvs =" all.ncdump | awk 'BEGIN {FS=":"} {print $1+1}'`
d2=`wc -l all.ncdump | awk '{print $1-1}'`
...
if [ $? -ne 0 ]
then
    echo "C Code is not compiled !"
    exit 1;
fi

${EXECDIR}/pre_${MODEL_Preprocess}.out
