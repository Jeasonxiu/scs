...
mysql -u shule << EOF
create database if not exists ${SYNDB};
EOF
...
Rad_Pat_S    double comment "Fake S Radiation Pattern for synthesis.",
Rad_Pat_ScS  double comment "Fake ScS Radiation Pattern for synthesis."
);
EOF
...
mysql -u shule ${SYNDB} << EOF
drop table if exists Master_a31;
create table Master_a31 as select * from Master_$$;
drop table if exists Master_$$;
EOF
