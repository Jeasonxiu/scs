...
# ===========================================================
# Plot Catalog for ScS ESW on all data.
#
# Shule Yu
# Oct 19 2014
# ===========================================================
...
do
    ...
    # =========================================
	INFILE="${WORKDIR_ESFAll}/${EQ}_${MainPhase}/INFILE"
    if ! [ -e ${INFILE} ]
    then
        echo "    ~=> `basename $0`: Run ESFAll first on ${EQ}..."
        continue
    else
        echo "    ==> Plotting ${MainPhase}_All catalog: ${EQ}..."
    fi

    # ESF Parameters.
    E1=`grep ${EQ} ${WORKDIR}/EQ_ESW_${RunNumber} | awk '{print $4}'`
    E2=`grep ${EQ} ${WORKDIR}/EQ_ESW_${RunNumber} | awk '{print $5}'`
...
select stnm,concat("${WORKDIR_ESFAll}/${EQ}_${MainPhase}/",stnm,".waveform"),netwk,gcarc,az,baz,stlo,stla,D_T_ScS_All,CCC_ScS_All,SNR_ScS_All,Weight_ScS_All,Misfit_ScS_All,Peak_ScS_All,Polarity_ScS_All,Rad_Pat_ScS,Norm2_ScS_All,N_T1_ScS_All,N_T2_ScS_All,S_T1_ScS_All,S_T2_ScS_All from Master_a10 where eq=${EQ} and wantit=1 order by gcarc;
EOF
    ...
    while read STNM file NETNM Gcarc AZ BAZ STLO STLA D_T CCC SNR Weight Misfit Peak Polarity Rad_Pat Norm2 N_T1 N_T2 S_T1 S_T2
    do
...
EOF
            ...
            page=$(($page+1))
            OUTFILE=${page}.ps
            title1="${MM}/${DD}/${YYYY}  DATA_CENTER: Merged PHASE: ${MainPhase} COMP: ${COMP}  Page: ${page}"
...
EOF
        ...
        awk -v S=${D_T} -v A=${AMP} -v P=${Polarity} '{print $1+S,$2*P/A}' ${WORKDIR_ESFAll}/${EQ}_${MainPhase}/${EQ}.ESF_F |  psxy -J -R -W0.3p,${color[1]},- -O -K >> ${OUTFILE}
        #### waveform
        awk -v T1=${PLOTTIMEMIN_Catalog_ScS} -v T2=${PLOTTIMEMAX_Catalog_ScS} -v A=${AMP} '{ if (  $1>T1 && $1<T2 ) print $1,$2/A}' ${file} | psxy -J -R -W0.5p -O -K >> ${OUTFILE}
