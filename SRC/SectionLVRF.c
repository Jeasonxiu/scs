...
//     enum Penum  {D1,D2,D3,D4};
    ...
    double lon,lat,depth,v_model,v_prem,dist_high,dist_whole,lon_previous,lat_previous,depth_previous,RE,dist,dv,vslow;
    ...
    for (count=0;count<2*(double_num-1);count++){
        ...
        sprintf(tmpstr,"%s.SectionLVRF_S_%d",PS[EQ],count+1);
        fpout=fopen(tmpstr,"w");
        fprintf(fpout,"<STNM> <Fraction>\n");

        sprintf(tmpstr,"%s.SectionLVRF_ScS_%d",PS[EQ],count+1);
        fpout1=fopen(tmpstr,"w");
        fprintf(fpout1,"<STNM> <Fraction>\n");

        while (fscanf(fp,"%lf%s%s%s",&vslow,stnm,Spathfile,ScSpathfile)==4){
            ...
            while (fscanf(fpin,"%lf%lf%lf",&lon,&lat,&depth)==3){
                ...
                if (count<double_num-1){
                    if ( dv < vslow && depth_previous <= depth && P[count] <= depth && depth <= P[count+1] ){
                        dist_high+=dist;
                    }
                }
                else{
                    if ( dv < vslow && depth_previous >= depth && P[2*(double_num-1)-count-1] <= depth && depth <= P[2*(double_num-1)-count]){
                        dist_high+=dist;
                    }
