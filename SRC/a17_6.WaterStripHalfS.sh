...
# Also, we strip the first Half of Deconed S from the second
# half to compensate the downswing near S for large distance stations.
#
# Outputs:
#
#           ${WORKDIR_WaterHalfSDecon}/${EQ}/
#           ScS.Master_a17
#
# Shule Yu
# Jun 22 2014
...
EOF
	...
	rm -rf ${WORKDIR_WaterHalfSDecon}/${EQ}
    mkdir -p ${WORKDIR_WaterHalfSDecon}/${EQ}
    cd ${WORKDIR_WaterHalfSDecon}/${EQ}
    cp ${WORKDIR}/tmpfile_INFILE_${RunNumber} ${WORKDIR_WaterHalfSDecon}/${EQ}/INFILE
    trap "rm -rf ${WORKDIR_WaterHalfSDecon}/${EQ} ${WORKDIR}/*_${RunNumber}; exit 1" SIGINT
...
EOF
        ...
        ${EXECDIR}/WaterHalfS.out 3 2 16 << EOF
${cate}
...
EOF
	...
	# Clean up.
    rm -f ${WORKDIR_WaterHalfSDecon}/${EQ}/tmpfile*
