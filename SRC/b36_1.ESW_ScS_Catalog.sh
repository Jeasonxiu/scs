...
select evlo,evla,evde,mag from Master_a36 where eq=${EQ} limit 1;
EOF
...
select stnm,concat("${WORKDIR_ESF}/${EQ}_${MainPhase}/",category,"/",stnm,".waveform"),netwk,gcarc,az,baz,stlo,stla,D_T_ScS,CCC_ScS,SNR_ScS,Weight_ScS,Misfit_ScS,Peak_ScS,Polarity_ScS,Rad_Pat_ScS,Norm2_ScS,N_T1_ScS,N_T2_ScS,S_T1_ScS,S_T2_ScS,category from Master_a36 where eq=${EQ} and wantit=1 order by category,gcarc;
EOF
