...
//     return 1;
    ...
    // Job begin.
    int    polarity;
    double gcarc,time,amp,xposition,yposition,shift;
    char   filename[300];
    FILE   *fp,*fpin,*fpout;

    fp=fopen(PS[infile],"r");
    fpout=fopen(PS[outfile],"w");
    while (fscanf(fp,"%lf%d%lf%s",&gcarc,&polarity,&shift,filename)==4){

        fpin=fopen(filename,"r");
        while (fscanf(fpin,"%lf%lf",&time,&amp)==2){
            time+=shift;
            if (time>P[timemin] && time<P[timemax]){
                xposition=P[timemin]+time-P[timemin];
                yposition=gcarc+P[range]/2*amp*polarity;
                fprintf(fpout,"%lf\t%lf\n",xposition,yposition);
            }
        }
        fprintf(fpout,">\n");
