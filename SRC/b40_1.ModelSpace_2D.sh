...
EOF
		...
		gmt psxy -JX`echo "0.9*${PropertyX_INC}*${scaleX}"|bc -l`i/`echo "0.4*${PropertyY_INC}*${scaleY}"| bc -l`i -R${TIMEMIN_MS}/${TIMEMAX_MS}/-0.5/0.5 -X0.1i -Y`echo "0.3 ${PropertyY_INC} ${scaleY}"|awk '{print $1*$2*$3}'`i -O -K >> ${OUTFILE} << EOF
EOF
