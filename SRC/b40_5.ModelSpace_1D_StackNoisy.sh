...
select concat(pairname,".frs") from Master_a38 where wantit=1 and gcarc=${GCARC};
EOF
...
EOF
    ...
    do
		file=`grep ${Model} filenames`
 		...
 		### stack FRSs from different batches.
 		rm -f tmpfile_filelist
 		for Batch in `seq 1 10`
 		do
 			echo "/home/shule/PROJ/t041.1D_Batch${Batch}.Noisy3/WaterFRS/${file}" >> tmpfile_filelist
 		done
 
 		${EXECDIR}/StackThem.out 0 4 1 << EOF
tmpfile_filelist
tmpfile_stack
tmpfile_std_up
tmpfile_std_down
...
EOF
        ...
        ### plot FRS's.
 		gmt psxy tmpfile_stack -J -R -W0.5p,black -O -K >> ${OUTFILE}
 		gmt psxy tmpfile_std_up -J -R -W0.5p,red -O -K >> ${OUTFILE}
 		gmt psxy tmpfile_std_down -J -R -W0.5p,red -O -K >> ${OUTFILE}
