...
int main(int argc, char **argv){
    ...
    double time,peak,**scs,**scs_fft_amp,**scs_fft_phase,**scs_divide_amp,**scs_divide_phase,*esf,*filled_esf_fft_amp,*filled_esf_fft_phase,**decon,*Nanchor,ccc,misfit,snr,snr_1,snr_2,df,AMP,*sPeakTime;
    ...
    sPeakTime=(double *)malloc(PI[fileN]*sizeof(double));
    ...
    for (count=0;count<PI[fileN];count++){
        ...
        fscanf(fp,"%s%s%lf%lf%lf",stnm[count],ScSfile,&peak,&Nanchor[count],&sPeakTime[count]);
    ...
    // Output deconed result. (ScS Peak is at the center, t=0)
    for (count=0;count<PI[fileN];count++){

		// Find deconed ScS peak.
		ScSPeak=NPTS_signal;
		findpeak(decon[count],2*NPTS_signal,&ScSPeak,(int)(-1/P[delta]),(int)(2/P[delta]));

		// Find deconed S peak.
		SPeak=NPTS_signal+(int)(sPeakTime[count]/P[delta]);
		findpeak(decon[count],2*NPTS_signal,&SPeak,(int)(-5/P[delta]),(int)(10/P[delta]));
        ...
        for (count2=0;count2<2*NPTS_signal;count2++){
			...
			// Use the first half of S to compensate the second half.
			if (SPeak<=count2 && count2<SPeak+(int)(35/P[delta])){
				fprintf(fpout,"%.4lf\t%.5e\n",(count2-ScSPeak)*P[delta],decon[count][count2]-decon[count][2*SPeak-count2]);
			}
			else{
				fprintf(fpout,"%.4lf\t%.5e\n",(count2-ScSPeak)*P[delta],decon[count][count2]);
			}
