...
#==============================================================
# This script: Deconvolve ScS ESF from each S traces, to evaluate
...
for EQ in ${EQnames}
do
    echo "    ==> EQ ${EQ}. S Deconvolution begin ! ( ${MainPhase}_${COMP} )"

	rm -rf ${WORKDIR_DeconScS}/${EQ}
    mkdir -p ${WORKDIR_DeconScS}/${EQ}
    cd ${WORKDIR_DeconScS}/${EQ}
    cp ${WORKDIR}/tmpfile_INFILE_${RunNumber} ${WORKDIR_DeconScS}/${EQ}/INFILE
    trap "rm -rf ${WORKDIR_DeconScS}/${EQ} ${WORKDIR}/*_${RunNumber}; exit 1" SIGINT

    for cate in `seq 1 ${CateN}`
    do
        # Judge whether Stretched S ESF has been made.
        if ! [ -d ${WORKDIR_ESF}/${EQ}_${MainPhase} ]
        then
            echo "    ~=> ESF of ${MainPhase} of ${EQ}, cate ${cate} doesn't exist ..."
            continue
        fi
        ...
        do
            echo ${stnm} ${WORKDIR_ESF}/${EQ}_${MainPhase}/${cate}/${stnm}.waveform ${peak} ${Nanchor} >> tmpfile_infile_${cate}
        done < tmpfile_$$
...
EOF
        if [ $? -ne 0 ]
        then
			echo "    !=> Decon: ${EQ} ScS Deconvolution C code failed on Category ${cate}..."
            continue
        fi
	...
	# Clean up.
    rm -f ${WORKDIR_DeconScS}/${EQ}/tmpfile*
