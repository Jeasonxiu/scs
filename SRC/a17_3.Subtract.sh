...
EOF
    ...
    echo "    ==> ${EQ} Subtraction begin ..."

	rm -rf ${WORKDIR_SubtractDecon}/${EQ}
    mkdir -p ${WORKDIR_SubtractDecon}/${EQ}
    cd ${WORKDIR_SubtractDecon}/${EQ}
    cp ${WORKDIR}/tmpfile_INFILE_${RunNumber} ${WORKDIR_SubtractDecon}/${EQ}/INFILE
    trap "rm -rf ${WORKDIR_SubtractDecon}/${EQ} ${WORKDIR}/*_${RunNumber}; exit 1" SIGINT
...
EOF
        ...
        ${EXECDIR}/Subtract.out 2 2 9 << EOF
${cate}
...
EOF
        if [ $? -ne 0 ]
        then
            echo "    !=> ${EQ}_Category${cate} Subtraction C code failed ..."
            continue
        fi
...
SNR_W1       double comment "Subtract SNR, signal is the peak, noise is right before signal window",
SNR_W2       double comment "Subtract SNR, signal is the peak, noise is right after signal window",
SNR_D        double comment "Subtract SNR, signal is the peak, noise is the same window as ESW noise window",
...
EOF
	...
	# Clean up.
    rm -f ${WORKDIR_SubtractDecon}/${EQ}/tmpfile*
