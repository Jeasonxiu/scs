...
# ===========================================================
# Plot profile of S, on different category. 
#
# Shule Yu
# Oct 19 2014
# ===========================================================
...
XSkip=`echo "${XSIZE}/${CateN}" | bc -l`
XSIZE=`echo "${XSkip}*0.8" | bc -l`
...
select evlo,evla,evde,mag from Master_a12 where eq=${EQ} limit 1;
EOF
    ...
    do
		...
		mysql -N -u shule ${DB} > tmpfile_$$ << EOF
select gcarc,D_T_S,Polarity_S,concat("${WORKDIR_ESF}/${EQ}_${ReferencePhase}/${Cate}/",stnm,".waveform") from Master_a12 where eq=${EQ} and wantit=1 and category=${Cate};
EOF
...
EOF
    ...
    # Title
    pstext -Jx1i -R0/6/0/9 -K -N -P -Y1.5i > ${OUTFILE} << EOF 
3.0 9.3 20  0 0 CB Event: ${MM}/${DD}/$YYYY ${HH}:${MIN} Phase=${ReferencePhase} Comp=T
3.0 9.0 15  0 0 CB ${EQ}  LAT=${EVLA} LON=${EVLO} Z=${EVDE} Mb=${EVMA}
-0.5 4.5 15 90 0 CB Distance (deg)
3.0 -0.7 15  0 0 CB Time after ${ReferencePhase} onset decided by ESW (sec)
EOF
    ...
    paste datetag1 datetag2 | pstext -Jx -R -N -Wored -G0 -O -K >> ${OUTFILE}

    # plot the records
    for Cate in `seq 1 ${CateN}`
    do
        psxy xy.seismograms_${Cate} -J${SCALE} -R${RANGE} -W0.005i,${color[${Cate}]} -B"$BAXIS"WSne -m -O -K >> ${OUTFILE}
        psxy -J -R -O -K -X${XSkip}i >> ${OUTFILE} << EOF
EOF
    done
