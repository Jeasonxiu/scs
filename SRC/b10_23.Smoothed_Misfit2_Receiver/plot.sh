...
do
    ...
    echo "    ==> Plotting Smoothed Misfit2 Distribution of ${EQ}..."
...
select stlo,stla,Misfit2_S_All,Misfit2_ScS_All from Master_a10 where eq=${EQ} and wantit=1;
EOF
...
# Misfit.
mysql -N -u shule ${DB} > tmpfile_stlo_stla_MisfitS_MisfitScS << EOF
select stlo,stla,Misfit2_S_All,Misfit2_ScS_All from Master_a10 where wantit=1;
EOF
