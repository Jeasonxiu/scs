...
EOF
    ...
    do
		...
		mysql -N -u shule ${DB} > tmpfile_$$ << EOF
select gcarc,D_T_ScS,Polarity_ScS,concat("${WORKDIR_ESF}/${EQ}_${MainPhase}/${Cate}/",stnm,".waveform") from Master_a13 where eq=${EQ} and wantit=1 and category=${Cate};
EOF
...
3.0 -0.7 15  0 0 CB Time after ${MainPhase} onset decided by ESW (sec)
EOF
