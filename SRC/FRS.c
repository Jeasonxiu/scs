...
//     enum PIenum {};
    ...
    amp=(double *)malloc((int)ceil(2*P[Time]/P[DELTA])*sizeof(double));
    ...
    while (fscanf(fp,"%s%s",in,out)==2){
        ...
        if (count%2==1){
            for (count2=count;count2>count/2;count2--){
                amp[count2]=amp[count2-1];
            }
            count++;
        }
        npts=count;

        frs=(double *)malloc(npts/2*sizeof(double));

        // Flip-Reverse-Sum.
        for (count=0;count<npts/2;count++){
            frs[count]=amp[npts/2+count]-amp[npts/2-count];
        }

        // Output the retrended FRS trace.
        fpout=fopen(out,"w");
        for (count=0;count<npts/2;count++){
