...
int main(int argc,char **argv){
    ...
    // Read in S and ScS esf fullstack. ( only between C1 and C2 )
    fpin=fopen(PS[Sesf],"r");
    count=0;
    while(fscanf(fpin,"%lf%lf",&time,&amp)==2){
        if (time>P[C1] && time<P[C2]){
    ...
    for (count=0;count<PI[nStretch];count++){
        ...
        abs_difference=0;
        if (shift>=0){
            OverlapLength=(int)fmin(NPTS_S_CC-shift,NPTS_ScS_CC);
            for(count2=0;count2<OverlapLength;count2++){
                abs_difference+=fabs(S_amp_CC[count2+shift]-ScS_amp_CC[count2]);
            }
        }
        else {
            OverlapLength=(int)fmin(NPTS_ScS_CC+shift,NPTS_S_CC);
            for(count2=0;count2<OverlapLength;count2++){
                abs_difference+=fabs(S_amp_CC[count2]-ScS_amp_CC[count2-shift]);
            }
        }
