...
do
    ...
    echo "    ==> Plotting Smoothed Misfit4 Distribution of ${EQ}..."
...
select stlo,stla,Misfit4_S_All,Misfit4_ScS_All from Master_a10 where eq=${EQ} and wantit=1;
EOF
...
# Misfit.
mysql -N -u shule ${DB} > tmpfile_stlo_stla_MisfitS_MisfitScS << EOF
select stlo,stla,Misfit4_S_All,Misfit4_ScS_All from Master_a10 where wantit=1;
EOF
