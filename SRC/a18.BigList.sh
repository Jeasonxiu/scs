...
for EQ in ${EQnames}
do
	echo "    ==> Generating ${EQ} information..."
    ...
    # 1. Station INFO_All.
    keys="<STNM> <EQ> <NETWK> <GCARC> <SHIFT_GCARC> <AZ> <BAZ> <STLO> <STLA> <EVLO> <EVLA>"
    ...
    do
        keys="<STNM> <D_T> <CCC> <SNR> <Weight> <Peak> <Rad_Pat> <Misfit> <Norm2> <Polarity> <WaterLevel> <Amplitude>"
        ...
        keys="<STNM> <D_T> <CCC> <SNR> <Weight> <Peak> <Rad_Pat> <Misfit> <Norm2> <Polarity> <Amplitude>"
    ...
    paste tmpfile_paste_1 tmpfile_paste_2 tmpfile_paste_3 tmpfile_paste_4 tmpfile_paste_5 > tmpfile_${EQ}

done # End of EQ loop. done INFO creation.

# Merge all of the estimations. Make INFO_All file.
keys1="<STNM> <EQ> <NETNM> <GCARC> <SHIFT_GCARC> <AZ> <BAZ> <STLO> <STLA> <EVLO> <EVLA>"
keys2="<D_T_S> <CCC_S> <SNR_S> <Weight_S> <Peak_S> <Rad_Pat_S> <Misfit_S> <Norm2_S> <Polarity_S> <WaterLevel> <Amplitude_S>"
keys3="<D_T_ScS> <CCC_ScS> <SNR_ScS> <Weight_ScS> <Peak_ScS> <Rad_Pat_ScS> <Misfit_ScS> <Norm2_ScS> <Polarity_ScS> <Amplitude_ScS>"
keys4="<HITLO> <HITLA>"
keys5="<Cate> <SNR_1> <SNR_2> <SNR_D> <CCC_St> <Misfit_St> <Shift_St>"
echo "${keys1} ${keys2} ${keys3} ${keys4} ${keys5}" > INFO_All
