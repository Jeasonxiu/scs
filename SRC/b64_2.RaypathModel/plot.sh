...
do
    ...
    do
        ...
        ${BASHCODEDIR}/Findfield.sh ${WORKDIR_Structure}/${TomoModel}/${EQ}.Count "${keys}" > tmpfile_$$
        ${BASHCODEDIR}/Findrow.sh tmpfile_$$ tmpfile_stnm | awk '{print $2}' > tmpfile_percentage

        keys="<STNM> <Vs>"
        ${BASHCODEDIR}/Findfield.sh ${WORKDIR_Structure}/${TomoModel}/${EQ}.Extreme "${keys}" > tmpfile_$$
        ...
        ${BASHCODEDIR}/Paste.sh tmpfile_category tmpfile_percentage tmpfile_extreme > tmpfile_cate_HVRF_EHV
...
EOF
    ...
    ps2pdf tmp.ps ${WORKDIR_Plot}/Structure_${MODELname}_${StructurePhase}_2.pdf
