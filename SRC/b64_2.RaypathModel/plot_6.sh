...
do
    awk -v C=${count} '{if ($1==C) print $2}' tmpfile_cate_per > histo.dat_${count}
...
pshistogram histo.dat_2 ${REG} ${PROJ} -W${XINC} -L0.5p -Ggreen -Ba${XNUM}f${XINC}:"${xlabel}":/a${YNUM}f${YINC}:"${ylabel}":WS -O -K >> ${OUTFILE}
