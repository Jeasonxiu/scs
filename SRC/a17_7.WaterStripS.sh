...
# Also, we strip the Deconed_S_ESW from the Deconed_S to
# compensate the downswing near S for large distance stations.
# Need Deconed_S_ESW, therefore need to check ${WORKDIR_WaterDecon}/${EQ}/ESW_S
#
# Outputs:
#
#           ${WORKDIR_WaterSDecon}/${EQ}/
#           ScS.Master_a17
#
# Shule Yu
# Jun 22 2014
...
EOF
	...
	rm -rf ${WORKDIR_WaterSDecon}/${EQ}
    mkdir -p ${WORKDIR_WaterSDecon}/${EQ}
    cd ${WORKDIR_WaterSDecon}/${EQ}
    cp ${WORKDIR}/tmpfile_INFILE_${RunNumber} ${WORKDIR_WaterSDecon}/${EQ}/INFILE
    trap "rm -rf ${WORKDIR_WaterSDecon}/${EQ} ${WORKDIR}/*_${RunNumber}; exit 1" SIGINT
    ...
    # Check Deconed_S_ESW result.
    if ! [ -s ${WORKDIR_WaterDecon}/ESW_S/${EQ}/${EQ}.ESF_F ]
    then
        echo "    !=> Deconed_S_ESW of ${EQ} doesn't exist ..."
        continue
    fi
...
select STNM,concat("${WORKDIR_ESF}/${EQ}_${MainPhase}/${cate}/",STNM,".waveform"),Peak_ScS,NA_ScS,S-ScS+Peak_S-Peak_ScS+D_T_S from Master_$$ where wantit=1 and eq=${EQ} and Category=${cate};
EOF
...
EOF
	...
	# Clean up.
    rm -f ${WORKDIR_WaterSDecon}/${EQ}/tmpfile*
