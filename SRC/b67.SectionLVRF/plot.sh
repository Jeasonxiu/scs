...
do
    ...
    do
        ...
        ${BASHCODEDIR}/Findfield.sh ${WORKDIR_Structure}/${TomoModel}/${EQ}.SectionLVRF_S_1 "<STNM>" | awk '{print $1}' | sort -u > tmpfile2_$$
        ...
        do
            keys="<STNM> <Fraction>"
            ${BASHCODEDIR}/Findfield.sh ${WORKDIR_Structure}/${TomoModel}/${EQ}.SectionLVRF_S_${count} "${keys}" > tmpfile_$$
            ${BASHCODEDIR}/Findrow.sh tmpfile_$$ tmpfile_stnm | awk '{ print $2}' > tmpfile_extreme_s_${count}

            keys="<STNM> <Fraction>"
            ${BASHCODEDIR}/Findfield.sh ${WORKDIR_Structure}/${TomoModel}/${EQ}.SectionLVRF_ScS_${count} "${keys}" > tmpfile_$$
...
EOF
    ...
    ps2pdf tmp.ps ${WORKDIR_Plot}/Structure_${MODELname}_SectionLVRF.pdf
