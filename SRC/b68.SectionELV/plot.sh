...
do
    ...
    do
        ...
        ${BASHCODEDIR}/Findfield.sh ${WORKDIR_Structure}/${TomoModel}/${EQ}.SectionL_S_1 "<STNM>" | awk '{print $1}' | sort -u > tmpfile2_$$
        ...
        do
            keys="<STNM> <Vs>"
            ${BASHCODEDIR}/Findfield.sh ${WORKDIR_Structure}/${TomoModel}/${EQ}.SectionL_S_${count} "${keys}" > tmpfile_$$
            ${BASHCODEDIR}/Findrow.sh tmpfile_$$ tmpfile_stnm | awk '{ print $2}' > tmpfile_extreme_s_${count}

            keys="<STNM> <Vs>"
            ${BASHCODEDIR}/Findfield.sh ${WORKDIR_Structure}/${TomoModel}/${EQ}.SectionL_ScS_${count} "${keys}" > tmpfile_$$
...
EOF
    ...
    ps2pdf tmp.ps ${WORKDIR_Plot}/Structure_${MODELname}_SectionELV.pdf
