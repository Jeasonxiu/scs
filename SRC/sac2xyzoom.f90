...
REAL(kind=8)                  :: time_beg_real
REAL                          :: sacamp(MAXnpts), cuty(MAXnpts),gcarc(MAXsac),  sacbeg, sacdt,sacgcarc
REAL                          :: tref
REAL, ALLOCATABLE             :: y(:,:), beg(:), dt(:), cutbeg(:), intp_y(:,:), intp_time(:,:)
REAL, ALLOCATABLE             :: reftime(:), newbeg(:)
...
CHARACTER*10                  :: refphase, t_header
   ...
   ALLOCATE( reftime(nSAC), newbeg(nSAC), STAT = keep)
   IF (keep /=0 ) STOP '<< ALLOCATE ERROR IN nSAC >>'
   ...
   do iSAC = 1, nSAC
       ...
       do ipts=1,PLOTnpts(iSAC)
           if ( intp_time(ipts,iSAC) .ge. beg(iSAC)-reftime(iSAC) .and. &
                intp_time(ipts,iSAC) .lt. beg(iSAC)-reftime(iSAC)+real(npts(iSAC))*dt(iSAC) ) then
                write(12,*) intp_time(ipts,iSAC), intp_y(ipts,iSAC)*amp_multiply+gcarc(iSAC), ipts 
           end if
       end do
   end do
   close(12)
   ...
   open(13, file = "xyz.seismograms")

   do iSAC = 1, nSAC
       write(13,'(''> '',2(f8.3))')  gcarc(iSAC), dt(iSAC)
       do ipts=1,PLOTnpts(iSAC)
           if ( intp_time(ipts,iSAC) .ge. beg(iSAC)-reftime(iSAC) .and. &
                intp_time(ipts,iSAC) .lt. beg(iSAC)-reftime(iSAC)+real(npts(iSAC))*dt(iSAC) ) then
                write(13,*) intp_time(ipts,iSAC), gcarc(iSAC) , intp_y(ipts,iSAC)
           end if
       end do
   end do
