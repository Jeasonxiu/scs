...
select stnm,file,stnm,${ReferencePhase},${N_A_S},Rad_Pat_${ReferencePhase} from Master_$$ where eq=${EQ} and WantIt=1;
EOF

	${BASHCODEDIR}/Findrow.sh tmpfile_$$ /home/shule/eq1_eq2_stnm | awk '{$1=""; print $0}' > tmpfile_Cin_$$
...
EOF
	...
	mysql -u shule ${DB} << EOF
update Master_$$ set WantIt=0 where eq=${EQ} and ( weight_S_all is null or Weight_S_All=0 );
EOF
	# deal with bad data
