...
EOF
	...
	# Trace info.
	mysql -N -u shule ${DB} > tmpfile_$$ << EOF
select stnm,concat("${WORKDIR_ESFAll}/${EQ}_${ReferencePhase}/",stnm,".waveform"),netwk,gcarc,az,baz,stlo,stla,D_T_S_All,CCC_S_All,SNR_S_All,Weight_S_All,Misfit_S_All,Peak_S_All,Polarity_S_All,Rad_Pat_S,Norm2_S_All,N_T1_S_All,N_T2_S_All,S_T1_S_All,S_T2_S_All from Master_a10 where eq=${EQ} and wantit=1;
EOF
	${BASHCODEDIR}/Findrow.sh tmpfile_$$ /home/shule/eq1_eq2_stnm > sorted.lst
