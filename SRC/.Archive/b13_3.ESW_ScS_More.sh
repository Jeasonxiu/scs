...
do
    ...
    # =========================================
    INFILE=${WORKDIR_ESF}/${EQ}_${MainPhase}_${COMP}/INFILE
    ...
    DATAFILE=`ls ${WORKDIR_ESF}/${EQ}_${MainPhase}_${COMP}/*DT`

    # Plot Parameters.
    OUTFILE=tmp.ps
    CateN=`grep "<CateN>"                                ${INFILE} | awk '{print $2}'` 
    E1=`grep "<${MainPhase}_E>"                          ${INFILE} | awk '{print $2}'`
    E2=`grep "<${MainPhase}_E>"                          ${INFILE} | awk '{print $3}'`
    ...
    do
        paste `ls ${WORKDIR_ESF}/${EQ}_${MainPhase}_${COMP}/*F${count}` `ls ${WORKDIR_ESF}/${EQ}_${MainPhase}_T/*F${count}.std` > tmpfile1_$$
        awk '{print $1,+$4}' tmpfile1_$$ > tmpfile_std${count}
        awk '{print $1,-$4}' tmpfile1_$$ | tac >> tmpfile_std${count}
        NR_used=`grep "<Nrecord${count}_Used>" ${WORKDIR_ESF}/${EQ}_${MainPhase}_${COMP}/STDOUT | awk '{print $2}'`
        Nrecord=`grep "<Nrecord${count}_All>" ${WORKDIR_ESF}/${EQ}_${MainPhase}_${COMP}/STDOUT | awk '{print $2}'`
        echo ${count} `ls ${WORKDIR_ESF}/${EQ}_${MainPhase}_${COMP}/*F${count}` tmpfile_std${count} ${NR_used} ${Nrecord} >> esf_std_infile.lst
    done

    # Histograms.
    keys=`awk '{printf "<%s> ",$1}' ${WORKDIR}/${MainPhase}_file_${RunNumber}`
    ${BASHCODEDIR}/Findfield.sh ${DATAFILE} "${keys}" > tmpfile_$$
...
${E1} -1.2 10 0 0 LB  EQ: ${EQ} for ${MainPhase}. X-Y: time(sec.) - amp
EOF
...
EOF
    ps2pdf tmp.ps ${WORKDIR_Plot}/${EQ}_ESF_STD_${MainPhase}.pdf


    # 2. Histograms. 

    rm ${OUTFILE} 2>/dev/null
    title="${EQ} Histograms. ( Phase: ${MainPhase}, Weight>0 , NR:${NR} , CCClow: ${CCCOFF}, SNRLow: ${SNRLOW}, SNRhigh: ${SNRHIGH} )"
...
EOF
    ...
    do
        ...
        # fix a bug that if all data falls into one box phistogram will report an error.
        flag_bug=`minmax -C histo.dat | awk '{if ($1==$2) print 1; else print 0}'`
        if [ ${flag_bug} -eq 1 ]
        then
            echo  "-100" >> histo.dat 
        fi
...
EOF
    done < ${WORKDIR}/${MainPhase}_file_${RunNumber}

    # Make PDF.
    psxy -J -R -O >> ${OUTFILE} << EOF
EOF
    ps2pdf ${OUTFILE} ${WORKDIR_Plot}/${EQ}_ESF_Hist_${MainPhase}.pdf
