...
EOF
    ...
    echo "    ==> Using close to average S and ScS shape of ${EQ} ..."

	# Information collection.
	mysql -N -u shule ${DB} > tmpfile_$$ << EOF
select PairName,misfit_s_all,misfit_scs_all from Master_$$ where eq=${EQ} and WantIt=1 order by misfit_s_all;
EOF
...
(PairName,@tmp1,@tmp2)
set Category=if((abs(@tmp1)<0.5 and abs(@tmp2)<0.5),1,0);
EOF
