...
do
    ...
    echo "    ==> Plotting Misfit3 Source Distribution of ${EQ}..."
...
select az,asin(Srayp*${Vs}/(6371.0-evde))*180.0/PI(),-1.0*Misfit3_S_All from Master_a10 where eq=${EQ} and wantit=1 and Misfit3_S_All<=0;
EOF
	mysql -N -u shule ${DB} > tmpfile_az_takeoff_MisfitS_Fat << EOF
select az,asin(Srayp*${Vs}/(6371.0-evde))*180.0/PI(),Misfit3_S_All from Master_a10 where eq=${EQ} and wantit=1 and Misfit3_S_All>0;
EOF
	mysql -N -u shule ${DB} > tmpfile_az_takeoff_MisfitScS_Thin << EOF
select az,asin(ScSrayp*${Vs}/(6371.0-evde))*180.0/PI(),-1.0*Misfit3_ScS_All from Master_a10 where eq=${EQ} and wantit=1 and Misfit3_ScS_All<=0;
EOF
	mysql -N -u shule ${DB} > tmpfile_az_takeoff_MisfitScS_Fat << EOF
select az,asin(ScSrayp*${Vs}/(6371.0-evde))*180.0/PI(),Misfit3_ScS_All from Master_a10 where eq=${EQ} and wantit=1 and Misfit3_ScS_All>0;
EOF
