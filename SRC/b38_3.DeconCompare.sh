...
# ===========================================================
# Overlay Decon results of two given Outputs.
#
# Shule Yu
# Aug 10 2015
# ===========================================================
...
do
    ...
    keys="<STNM>"
    ${BASHCODEDIR}/Findfield.sh `ls ${WORKDIR_Decon}/${EQ}/*Info` "${keys}" > tmpfile_stnm

    keys="<STNM> <SNR_1> <SNR_2> <SNR> <Shift_St> <CCC_St> <Misfit_St> <Cate> <N1Time> <S1Time> <N2Time> <N3Time>"
    ${BASHCODEDIR}/Findfield.sh `ls ${WORKDIR_Decon}/${EQ}/*Info` "${keys}" | awk '{if ($6<0) $6=-$6; print $0}'> tmpfile_$$
    ...
    paste tmpfile_paste_1 tmpfile_paste_2 tmpfile_paste_3 tmpfile_paste_4 > tmpfile_$$

    # Sort traces.
    sort -g -k 1,1 tmpfile_$$ > sorted.lst
...
EOF
    ...
    ps2pdf tmp.ps ${WORKDIR_Plot}/${EQ}_Decon.pdf
