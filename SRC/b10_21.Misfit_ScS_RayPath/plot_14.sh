...
scale=`minmax -C tmpfile_ScS_Grid_${Num} | awk '{if (-$5>$6) print -$5; else print $6}'`
if [ ${scale} -eq 0 ]
then
	exit 0
fi
...
gmt xyz2grd tmpfile_ScS_Grid_${Num} -Gtmp.grd ${REG} -I1/1
gmt makecpt -Cpolar -T-${scale}/${scale}/`echo "${scale}" |awk '{print 2*$1/51}'` > tmpfile.cpt
...
gmt pscoast -JX${width}id/${height}id ${REG} -W0.2p,black -A20000 -Dh -O -K >> ${OUTFILE}
