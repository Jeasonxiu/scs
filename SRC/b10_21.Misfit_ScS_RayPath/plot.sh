...
# Misfit and ray path file.
mysql -N -u shule ${DB} > tmpfile_ScS_pathfile_misfit << EOF
select concat("${WORKDIR_Sampling}/",PairName,"_ScS.path"),Misfit_ScS_All from Master_a10 where wantit=1;
EOF

NR=`wc -l < tmpfile_ScS_pathfile_misfit`

echo "    ==> Plotting Misfit RayPath Layer for all Data (NR: ${NR})..."
