...
# ================================================================
# Plot Histogram of the weights for each geographic bins.
...
do
	...
	keys="<EQ> <STNM>"
	${BASHCODEDIR}/Findfield.sh ${file} "${keys}" | awk '{print $1"_"$2}' > tmpfile_pairname

	rm -f tmpfile.sql
	while read pairname
	do
		echo "select Weight_Final from Master_a21 where PairName=\"${pairname}\";" >> tmpfile.sql
	done < tmpfile_pairname

	mysql -N -u shule ${DB} > histo.dat < tmpfile.sql
	...
	#==========  some text  ==========
	cat > tmpfile_$$  << EOF
0 1.1 Bin${Bin} weights from waveform estimations. NR=`wc -l < histo.dat`
EOF
