...
gmt psxy tmpfile_MisfitScS_DTScS_Thin ${REG} ${PROJ} -Sx -Wblue -N -O -K >> ${OUTFILE}
gmt psxy tmpfile_MisfitScS_DTScS_Fat ${REG} ${PROJ} -Sc -Wred -N -O -K >> ${OUTFILE}

cat > tmpfile_$$ << EOF
${XMIN} ${YMIN} ScS Misfit-dT, Thin(@;blue;${NR1}@;;) + Fat(@;red;${NR2}@;;).
EOF
