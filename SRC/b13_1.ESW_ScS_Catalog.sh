...
do
    ...
    # =========================================
	INFILE="${WORKDIR_ESF}/${EQ}_${MainPhase}/1/INFILE"
    if ! [ -e ${INFILE} ]
    then
        echo "    ~=> `basename $0`: Run a12.ESW_S.sh first on ${EQ}..."
        continue
    else
        echo "    ==> Plot ScS catalog: ${EQ}."
    fi
...
select evlo,evla,evde,mag from Master_a13 where eq=${EQ} limit 1;
EOF
...
select stnm,concat("${WORKDIR_ESF}/${EQ}_${MainPhase}/",category,"/",stnm,".waveform"),netwk,gcarc,az,baz,stlo,stla,D_T_ScS,CCC_ScS,SNR_ScS,Weight_ScS,Misfit_ScS,Peak_ScS,Polarity_ScS,Rad_Pat_ScS,Norm2_ScS,N_T1_ScS,N_T2_ScS,S_T1_ScS,S_T2_ScS,category from Master_a13 where eq=${EQ} and wantit=1 order by category,gcarc;
EOF
