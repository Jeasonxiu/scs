 ...
 *
 *    Use different waterlevel for different earthquakes.
 *    WL is decided by pre-signal noise level.
 *
 * Shule Yu
 * Nov 03 2014
*************************************************************************/
...
int main(int argc, char **argv){
    ...
    enum Penum  {sigma,gwidth,delta,Taper_source,Taper_signal,C1,C2,N1,N2,S1,S2,AN,filter1,filter2};
    ...
    double time,peak,**scs,**scs_fft_amp,**scs_fft_phase,**scs_divide_amp,**scs_divide_phase,*esf,*filled_esf_fft_amp,*filled_esf_fft_phase,**decon,*Nanchor,ccc,misfit,snr,snr_1,snr_2,df,AMP,*waterlevel;
    ...
    waterlevel=(double *)malloc(PI[fileN]*sizeof(double));
    ...
    for (count=0;count<PI[fileN];count++){
        ...
        fscanf(fp,"%s%s%lf%lf%lf",stnm[count],ScSfile,&peak,&Nanchor[count],&waterlevel[count]);
    ...
    // Decon.
    waterlevel_decon_wl(scs,PI[fileN],NPTS_signal,esf,NPTS_source,P0,P2,decon,waterlevel,P[delta],1,filled_esf_fft_amp,filled_esf_fft_phase,scs_fft_amp,scs_fft_phase,scs_divide_amp,scs_divide_phase);
