...
int main(int argc,char **argv){
	...
	int NPTS_S_DIFF=WE-WB;
	double *S_amp_DIFF=(double *)malloc(NPTS_S_DIFF*sizeof(double));
		...
		for (count2=0;count2<PI[nXStretch];count2++){
			...
			if (shift>=0){
				...
				for(count3=0;count3<OverlapLength;count3++){
					abs_difference+=fabs(S_amp_CC[count3+shift]-ScS_amp_CC[count3]);
				}
			}
			else {
				OverlapLength=(int)fmin(NPTS_ScS_CC+shift,NPTS_S_CC);
				for(count3=0;count3<OverlapLength;count3++){
					abs_difference+=fabs(S_amp_CC[count3]-ScS_amp_CC[count3-shift]);
				}
			}
