...
cat tmpfile_MisfitS_DTS_Thin tmpfile_MisfitS_DTS_Fat | awk '{print $2}' > tmpfile_$$ 
...
gmt psxy tmpfile_MisfitS_DTS_Thin ${REG} ${PROJ} -Sx0.1i -Wblue -N -O -K >> ${OUTFILE}
gmt psxy tmpfile_MisfitS_DTS_Fat ${REG} ${PROJ} -Sc0.1i -Wred -N -O -K >> ${OUTFILE}
