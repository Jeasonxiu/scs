...
do
	...
	if ! [ -d ${WORKDIR_WaterDecon}/${EQ} ] || ! [ -d ${WORKDIR_SubtractDecon}/${EQ} ] || ! [ -d ${WORKDIR_RawDecon}/${EQ} ]
	then
...
EOF
	...
	mysql -N -u > tmpfile_info shule ${DB} << EOF
select STNM,NETWK,Weight_Final,GCARC,Category,D_T_S,CCC_S,Polarity_S,D_T_ScS,CCC_ScS,CCC_D,Polarity_ScS,HITLO,HITLA,Peak_S,Peak_ScS,Shift_D from Master_a20 where eq=${EQ} and wantit=1 order by gcarc;
EOF
    ...
    do
        ...
        Sesf_stretched_Water=${WORKDIR_WaterDecon}/${EQ}/${Cate}.esf
        deconfile_Water=${WORKDIR_WaterDecon}/${EQ}/${STNM}.trace
        Sesf_stretched_Subtract=${WORKDIR_SubtractDecon}/${EQ}/${Cate}.esf
        deconfile_Subtract=${WORKDIR_SubtractDecon}/${EQ}/${STNM}.trace
        Sesf_stretched_Raw=${WORKDIR_RawDecon}/${EQ}/${Cate}.esf
        deconfile_Raw=${WORKDIR_RawDecon}/${EQ}/${STNM}.trace

        frsfile_Water=${WORKDIR_WaterFRS}/${EQ}_${STNM}.frs
        frsfile_Subtract=${WORKDIR_SubtractFRS}/${EQ}_${STNM}.frs
        frsfile_Raw=${WORKDIR_RawFRS}/${EQ}_${STNM}.frs
...
EOF
            ...
            title2="ELAT/ELON: ${EVLA} ${EVLO}  Depth: ${EVDE} km. Mag: ${EVMA}  NSTA: `wc -l < sort.lst`"
...
-0.9 1 9 0 1 LT Waterlevel
EOF
            pstext ${PROJ} -R-1/1/-1/1 -X${onefourthwidth}i -N -O -K >> ${OUTFILE} << EOF
-0.9 1 9 0 1 LT Subtract
EOF
            pstext ${PROJ} -R-1/1/-1/1 -X${onefourthwidth}i -N -O -K >> ${OUTFILE} << EOF
-0.9 1 9 0 1 LT Raw 
EOF
...
# ${Peak_ScS} ${AMP_ScS}
# EOF
        awk -v C=${Polarity_ScS} '{print $1,$2*C}' ScS.xy | psxy ${PROJ} ${REGESF} -Wblack,- -O -K >> ${OUTFILE}
